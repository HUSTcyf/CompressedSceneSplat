...
class LangPretrainer(nn.Module):
    ...
    def forward(self, input_dict, chunk_size=None):
        ...
        if self.training:
            ...
            # 目标 = 数据集 lang_feat 原样（方案 X 已删除，2026-08-04 清理）
            target = input_dict["lang_feat"]
            ...
            loss_result = self.criteria(
                ...
                offset=input_dict.get("offset"),  # 2026-08-04: ClassMeanProcrustesLoss 场景边界
            )
    ...
    def _chunked_forward(self, input_dict, chunk_size):
        ...
        for start_idx in range(0, N, chunk_size):
            ...
            if is_training:
                # 目标 = 数据集 lang_feat 原样（方案 X 已删除，2026-08-04 清理）
                target = chunk_input_dict["lang_feat"]
                # Pass coord, Gaussian parameters, and scene_path for Rendered2DLoss
                loss = self.criteria(
                    ...
                    offset=chunk_input_dict.get("offset"),  # 2026-08-04: ClassMeanProcrustesLoss 场景边界
                )

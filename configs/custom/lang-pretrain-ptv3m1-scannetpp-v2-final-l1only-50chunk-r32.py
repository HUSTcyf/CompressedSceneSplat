...
# 2026-08-04 训练策略变更（用户定）：与 val 类别分布最相近的 50 个 train
# chunks（tools/compression/select_top50_chunks.py -> top50_chunks.txt，symlink
# 到 train_top50_grid1.0cm_chunk6x6_stride3x3/），loop=100 -> 每 chunk 恰好
# 迭代 100 遍（50x100=5000 样本，batch=4 -> 1250 iters/epoch），epoch=1。
# 基于最新 final-l1only 配方（2026-08-04 15:16 同步版）：noaug + v3 注册表 +
# 16dim-init + block lr 限速（param_dicts=[dict(keyword="block", lr=0.0006)]）
# + SVDWeightedL1Loss(inverse_variance) + CosineSimilarity +
# AggregatedContrastiveLoss(0.1, last_75)。训练输出不 normalize。
...
svd_rank = 32  # SVD compression rank (same as litept baseline)
...
# ============================================================================
epoch = 10  # 50-chunk 配方：10 epochs x 10 遍（每 10 遍测一次性能）
eval_epoch = 1
...
save_path = "exp/smoke-ptv3m1-16-scannetpp-v2-final-l1only-50chunk-r32"
...
data = dict(
    ...
    train=dict(
        ...
        loop=10,  # 2026-08-04 50-chunk 配方：每 epoch 10 遍（50x10=500 样本/epoch）
        # 2026-08-03 全局符号对齐注册表（加载时翻转列符号，原始 npz 不动）。
        # 生成：tools/compression/build_global_sign_registry.py（只读）。
        global_sign_path="/home/isom/cyf/SceneSplat/scannetpp_v2/lang_feat_grid_svd_r32_global_signs_v3.npz",
        transform=[

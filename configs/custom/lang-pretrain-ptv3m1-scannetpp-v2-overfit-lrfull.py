...
model = dict(
    ...
    # 2026-08-04 决定性消融：只保留加权 L1（去掉 cos + contrast）。
    # 若单 chunk raw L1 仍卡 ~0.068（MLP 上界 0.0188 的 3.6 倍）→ 训练代码有 bug；
    # 若收敛至 ~0.019 且幅度比→1.0 → 瓶颈 = 损失组合（cos/contrast 干扰），代码正确。
    criteria=[
        dict(
...
scheduler = dict(
    type="OneCycleLR",
    max_lr=0.006,  # 单组（block 满速）
    pct_start=0.5,  # 高 LR 窗口 300 iters
    anneal_strategy="cos",
    div_factor=10.0,
    final_div_factor=1000.0,
)
param_dicts = None  # 去掉 block 限速（block lr = 主 lr）

# Save path for PT-v3m1-16 training (compressed features)
save_path = "exp/overfit_lrfull"

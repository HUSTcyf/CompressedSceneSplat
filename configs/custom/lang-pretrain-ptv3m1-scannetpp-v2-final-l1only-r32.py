...
# 2026-08-04 REBUTTAL T1-3: 只变压缩 rank（r16 -> r32）。dec0 头 32 维，
# 16dim-init 权重中 dec0 被 CheckpointLoader 自动跳过，其余照常加载。
...
model = dict(
    ...
    # - 去掉 AggregatedContrastiveLoss（实测破坏：单 chunk raw L1 0.068→0.036、
    #   幅度 2.2→0.7、corr 0.90→0.95；多 chunk epoch5 突变激活致崩盘）
    # - 保留 SVDWeightedL1Loss（inverse_variance：波动维高权重）+ CosineSimilarity
    #   （消融证明 cos 无干扰，方向监督免费）
    criteria=[
        dict(
...
scheduler = dict(
    type="OneCycleLR",
    max_lr=0.006,  # 单组（param_dicts=None，block 满速）
    pct_start=0.1,  # 2026-08-04 修正：0.5 无依据（单 chunk 问题错套多 chunk），多 chunk 20120 iters 下 0.1=2000 iters 高 LR 足够
    anneal_strategy="cos",
    div_factor=10.0,
    final_div_factor=1000.0,
)
param_dicts = None  # 2026-08-04：去掉 block 限速（实测 block 满速收敛快 2.7 倍）

# Save path (2026-08-04: 正式 run，标准增强，不带 noaug)
save_path = "exp/smoke-ptv3m1-16-scannetpp-v2-final-l1only-r32"

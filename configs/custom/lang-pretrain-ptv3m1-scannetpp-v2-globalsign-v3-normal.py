...
model = dict(
    ...
    backbone=dict(
        type="PT-v3m1",
        in_channels=14,  # 3DGS + normal: color(3)+opacity(1)+quat(4)+scale(3)+normal(3)（原版 w-normal 配置）
...
save_path = "exp/smoke-ptv3m1-16-scannetpp-v2-globalsign-v3-normal"
...
data = dict(
    ...
    train=dict(
        ...
        transform=[
            ...
            # 2026-08-04：标准增强（与 smoke 一致）。2026-08-03 用户定论：
            # 去增强无用（noaug 1.61% < 增强 3.29%），正式配置保留标准增强。

...
def readCamerasFromTransforms(path, transformsfile, depths_folder, white_background, is_test, extension=".png"):
    ...
    with open(os.path.join(path, transformsfile)) as json_file:
        ...
        # Check if this is ScanNet format (uses fl_x) or Blender/NeRF format (uses camera_angle_x)
        if "camera_angle_x" in contents:
            # Blender/NeRF Synthetic format
            fovx = contents["camera_angle_x"]
            use_focal_length = False
        elif "fl_x" in contents:
            # ScanNet format - uses focal length in pixels
            focal_length = contents["fl_x"]
            use_focal_length = True
        else:
            raise ValueError("transforms file must contain either 'camera_angle_x' (Blender) or 'fl_x' (ScanNet)")
        ...
        for idx, frame in enumerate(frames):
            ...
            # For ScanNet format, don't flip Y and Z axes (already in COLMAP format)
            # For Blender/NeRF format, change from OpenGL/Blender camera axes to COLMAP
            ...
            # For Blender/NeRF format (.png with RGBA), process alpha channel
            # For ScanNet format (.jpg), skip alpha channel processing
            ...
            # Calculate FOV based on format
            if use_focal_length:
                # ScanNet format: use focal length in pixels
                FovY = focal2fov(focal_length, image.size[1])
                FovX = focal2fov(focal_length, image.size[0])
            else:
                # Blender/NeRF format: use camera_angle_x

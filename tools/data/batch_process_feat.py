...
# dst_path = "/home/isom/cyf/SceneSplat/projects/OccamLGS/output/LERF"
# feat_levels = [1]
for scene in scenes:
    ...
    for f in feat_levels:
        ...
        f" --lang_checkpoint /home/isom/cyf/CompressedSceneSplat/output_features/{scene}/checkpoint_with_features_p.pth --visualize --use_siglip_sam2_format"
        # cmd = f"CUDA_VISIBLE_DEVICES={device} python tools/data/feature_map_renderer.py -m {dst_path}/{scene} --iteration {iteration} --eval --skip_train --feature_level {f} --src_dim {src_dim} -r {res}" \
        # f" --lang_checkpoint /home/isom/cyf/CompressedSceneSplat/output_features/{scene}/checkpoint_with_features_s.pth --visualize"
        print(cmd)
        os.system(cmd)

        # cmd = f"CUDA_VISIBLE_DEVICES={device} python tools/assign_2d_labels_to_3d.py --dataset_type {dataset} \
        # --model_path /home/isom/cyf/CompressedSceneSplat/gaussian_results/{dataset}/{scene} \
        # --source_path /home/isom/cyf/SceneSplat/datasets/{dataset}/{scene} \
        # --output_dir /home/isom/cyf/SceneSplat/gaussian_train/{dataset}/train/{scene} --eval -r {res}"
        # print(cmd)
        # os.system(cmd)
    # break

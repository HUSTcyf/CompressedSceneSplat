...
class Runner:
        ...
        for step in pbar:
            ...
            try:
                try:
                    ...
                    trainloader_iter = iter(trainloader)
                    data = next(trainloader_iter)
                if data["image"] is None or (cfg.depth_loss and data.get("depth") is None):
            ...
            depth_gt = None
            if cfg.depth_loss and "depth" in data:
                # points = data["points"].to(device)  # [1, M, 2]
                # depths_gt = data["depths"].to(device)  # [1, M]
                depth_gt = data["depth"].to(device)  # [1, H, W]
            elif cfg.depth_loss and "depth" not in data:
                # Depth loss enabled but no depth data available (e.g., COLMAP)
                print(f"\033[93mWARNING: Depth loss enabled but no depth data in dataset. Skipping depth loss.\033[0m")
                cfg.depth_loss = False  # Disable depth loss for this run
            ...
            if step in [i for i in cfg.eval_steps]:
                ...
                self.save_checkpoint(
                    path=f"{self.ckpt_dir}/chkpnt{step}.pth"
                )
                if cfg.render_traj:
    ...
    def eval(self, step: int, stage: str = "val", eval_depth: bool = False):
        ...
        for i, data in enumerate(valloader):
            try:
                ...
                height, width = pixels.shape[1:3]
                if eval_depth and "depth" in data:
                    depth_gt = data["depth"].to(device)  # [1, H, W]
                elif eval_depth and "depth" not in data:
                    depth_gt = None  # No depth data available
    ...
    def save_checkpoint(self, path):
        ...
        # Prepare splats dictionary for saving
        splats_dict = {}
        for k in self.splats.keys():
            splats_dict[k] = self.splats[k].cpu()

        # Save checkpoint with splats state
        torch.save({
            "splats": splats_dict,

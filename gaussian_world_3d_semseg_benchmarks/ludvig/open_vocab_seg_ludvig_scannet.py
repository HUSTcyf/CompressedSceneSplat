...
def main():
    args = parse_args()
    args.use_dot_similarity = True  # default
    ...
    # ------------------------------
    # 3) Evaluate on both ScanNet20 & ScanNet200
    # ------------------------------
    benchmarks = {
    ...
    for bench_name, label_path in benchmarks.items():
        ...
        for scene_id in tqdm(scene_ids, desc="Scene", dynamic_ncols=True):
            ...
            # ---- 3.2.d Predict labels for *labeled subset* ----
            ...
            if args.save_pred:
                ...
                save_fname = f"ludvig_{model_name}_{bench_name.lower()}_{scene_id}_semseg_pred.npy"
